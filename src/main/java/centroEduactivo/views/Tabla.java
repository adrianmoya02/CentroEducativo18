...
import centroEduactivo.controladores.EstudianteControlador;
...
public class Tabla extends JPanel {
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON1) {
					...
					Estudiante est = (Estudiante) EstudianteControlador.getInstance().find(indiceFilaSel);
					
					PanelEstdiante panel = new PanelEstdiante(est);
	...
	private Object[][] getDatosDeTabla() {
		estudiantes = (List<Estudiante>) EstudianteControlador.getInstance().findAll();
